...
def parse_args():
    """Parse command-line arguments for Gumbel-Max watermark data preparation."""
    parser = argparse.ArgumentParser(description="Prepare data using Gumbel-Max watermark generation.")

    # Core watermark configuration
    parser.add_argument('--method', default="Gumbel", type=str, help="Watermarking method (default: Gumbel).")
...
def main():
    """Generate unique samples using Gumbel-Max watermark."""
    args = parse_args()
    ...
    exp_name = f"text_data/{model_name}-{args.method}-c{args.c}-m{args.m}-{args.seed_way}-{args.seed}-temp{args.temperature}-stream-unique.pkl"
